...
// 设置时区，用于修复上传服务异常的问题，主要是linux的时区和后台的时区可能不一样
...
let hooks = {
  checkAuth: async function (ctx) {
    // 检查 用户是否登录，在 await ctx.getUserInfo() 中 注入 userInfo
    // 如果没有登录，throw Error
...
const initialize = function () {
  ...
  app.config = loader.loadConfig([
    path.resolve(__dirname, './config/common'),
    path.resolve(__dirname, './config/' + app.env)
  ])
  // load utils
  app.utils = loader.loadPlugin(path.resolve(__dirname, './utils'))

  // load plugins
  const plugins = loader.loadPlugin(path.resolve(__dirname, './plugins'))
